...
const handleHelpDeskRequest = async (req: any, res: any) => {
    try {
        ...
        switch (method) {
            case 'POST':
                logger.log("POST Request", req.body)
                return HelpDeskService.processHelpDeskRequest(body.message).then((helpDeskResp) => {
